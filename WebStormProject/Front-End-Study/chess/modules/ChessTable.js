...
export class ChessBoard {
  ...
  initalizeBoard() { // 체스 테이블 초기화
    this.board = this.board.map((row, rowIndex) => {
      ...
      if (rowIndex === 1 || rowIndex === 6) {
        ...
        firstRow = row.map((col, colIndex) => new Pawn(rowIndex, colIndex, tmpColor, imojiColor[4]));
        return firstRow;
      }
  ...
  setPiece(targetPos, movePos, board) { // targetpos의 piece에 대해 이동 가능성 판별 -> 이동
    ...
    const movingAvailability = this.board[targetPos[1]][targetPos[0]].checkMovingAvailability(targetPos, movePos, board);
